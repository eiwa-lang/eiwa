...
- [ ] **Task 65.9 (Futuro — remover o resto do glue C do `std.http`):** Eliminar `curl_helpers.h`/`curl_helpers.c` e fazer o `lib NativeHttp` chamar o **curl diretamente**. **Progresso (Ago 2026):** com o FFI variádico da Phase 66, os helpers `eiwa_curl_setopt_{string,ptr,int}` foram **removidos** — o `Client` agora chama `curl_easy_setopt` direto via `curlEasySetopt(curl, option, value...)`. **Restam 4 helpers** por limitações distintas: (a) `eiwa_curl_get_status` — `curl_easy_getinfo` é varargs mas precisa de `&response_code` (endereço de variável local, que Eiwa ainda não expõe); (b) `eiwa_get_write_callback_ptr` — o write callback exige ponteiro de função C que lambda Eiwa não fornece; (c) `eiwa_curl_buf_data`/`eiwa_curl_buf_size` — leitura dos campos de `struct EiwaCurlBuffer` alocado no lado C (acesso a structs C). **Dívida imediata:** o `curl_helpers.c` duplica as funções `static inline` do header (duas fontes de verdade) — dedupe pendente (header → declarações apenas; `.c` → única implementação). **Objetivo:** callbacks C como cidadãos de primeira classe + acesso a structs C, removendo todo o `@Source` do http.
...
- [x] **Verify 66:** `zig build test` + guardrail `samples/tests` **55/55** (novo `varargs_test.ei` + `ffi_varargs_test.ei`) nos backends LLVM e C; `samples/varargs_sample.ei` roda; `http_test.ei` (GET/POST reais) passa nos dois backends com o `curl_easy_setopt` direto.
