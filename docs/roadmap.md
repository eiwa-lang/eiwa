...
- [~] **Task 61.5 (NÃO CONCLUÍDA — reaberta):** Remover special cases de `toString`/`hashCode`/`replace` no LLVM emitter, roteando via vtable real de `Stringable`/`Hashable`. **Ainda há 6 `TODO(emitter): SPECIAL CASE`**; os helpers `emitToStringHelper`/`emitHashStringHelper`/`emitStrReplaceHelper` continuam presentes. `String.replace` foi redirecionado a `eiwa_str_replace` (fix AGO/2026) mas continua como special case, não via vtable.
  > **Verificação AGO/2026 (tentativa de remoção da tolerância `core.zig:560`):** rodar o emitter em modo estrito (falhar em vez de stub) provou que a tolerância **ainda é necessária** — a paridade 59/59 não a removeu. Corpos reachable auto-gerados da stdlib que ainda falham: `IntVar.toString` (`get_expr .toString()` em `Pointer` puro — **corrigido**: `.Pointer` agora emite `eiwa_to_string`, espelhando o C), `IntVar.hashCode` (o `call_expr` re-invocava o resultado do `get_expr` como ponteiro de função — `call addrspace(64) i64 %hash()` → **corrigido**: pass-through de `hashCode` no call_expr, espelhando o pass-through de `toString`), e **`JsonValue.toString` (ainda aberto)** — `.toString()` em tipo enum/custom desce pelo caminho de closure (`expression.zig` ~3072), emitindo o callee `get_expr` direto e falhando `PropertyNotFound`. O caso enum/custom exige o trabalho estrutural de String (Task 64.11) ou dispatch via vtable real — não é um special-case simples. **Índice completo dos TODO restantes:** [docs/tasks-backend-parity.md](docs/tasks-backend-parity.md).
