...
- [ ] **Task 65.9 (Futuro — remover o resto do glue C do `std.http`):** Eliminar `curl_helpers.h`/`curl_helpers.c` e fazer o `lib NativeHttp` chamar o **curl diretamente**. **Progresso (Ago 2026):** o FFI variádico (Phase 66) removeu `eiwa_curl_setopt_{string,ptr,int}`; e os slots de memória `eiwa_load_int64`/`eiwa_store_int64` (`Standard.loadInt64`/`storeInt64`, helpers do runtime) permitiram chamar `curl_easy_getinfo` direto, removendo `eiwa_curl_get_status`. O http agora usa a API idiomática `status = Memory.alloc<IntVar> { NativeHttp.curlEasyGetInfo(curl, CURLINFO_RESPONSE_CODE, it.ptr) }.get()` (estilo Kotlin `memScoped`; ver `type IntVar` + `object Memory.alloc<T>` no `std/core.ei` — `T` é o tipo do slot, monomorfizado via construtor `T(ptr)`, então `alloc<DoubleVar>` funcionará quando esse slot existir). **Restam 3 helpers** por uma limitação: `eiwa_get_write_callback_ptr` + `eiwa_curl_write_callback` + `eiwa_curl_buf_data`/`eiwa_curl_buf_size` + `EiwaCurlBuffer` — o write callback precisa de um ponteiro de função C **e** do acesso aos campos do buffer. **Feature nova (Ago 2026): `funPointer { lambda }`** — builtin que gera um trampolim C (`eiwa_cb_<name>`) que encaminha para uma lambda inline sem captura (estilo Kotlin `staticCFunction`); lambdas grandes podem delegar para uma função nomeada; o trampolim é emitido nos dois backends e a função alvo é mantida reachable no LLVM. **Falta:** acesso a structs C (`struct` no lib block + `.as(T)`) para o corpo do callback escrever no `core_String` — aí `eiwa_curl_write_callback`/`eiwa_get_write_callback_ptr`/`buf_data`/`buf_size`/`EiwaCurlBuffer` somem. **Dívida imediata:** o `curl_helpers.c` duplica as funções `static inline` do header (duas fontes de verdade) — dedupe pendente (header → declarações apenas; `.c` → única implementação). **Objetivo:** callbacks C + acesso a structs C, removendo todo o `@Source` do http.
