...
> - Regra: no máximo uma função `@MainWrapper` por programa (senão erro de ambiguidade).
...
- [ ] **Task 65.4:** Anotação `@MainWrapper` — validar no type checker que é uma função (top-level, método de `object` ou método de `lib`) com a assinatura padrão `(Int, Pointer) -> Int` e no máximo uma por programa; rejeitar em método de instância de `type`.
- [ ] **Task 65.5:** Emitter LLVM — entry point renomeia o `main` real e emite a chamada à função `@MainWrapper`: função Eiwa → closure `{ptr @real_main, ptr null}`; método de lib → ponteiro C cru do `@Alias`. Shim `i32 (i32, ptr)` no modo teste.
- [ ] **Task 65.6:** C transpiler — migrar do `#define main` para a anotação `@MainWrapper` (mesma semântica, removendo o macro de preprocessador; suporta wrapper Eiwa e wrapper C de lib).
- [ ] **Task 65.7:** `src/std/coroutines.ei` — o `lib Neco` declara `@MainWrapper` + `@Alias("Neco_main_wrapper")`; mover o corpo do macro para `Neco_main_wrapper` em `neco_wrapper.c`; apagar o `#define main`.
- [x] **Task 65.8:** `task_test.ei` (12/12) e `http_test.ei` (2/2) passam no LLVM — movidos para `passing_llvm` (53 arquivos); guardrail 53/53 nos backends LLVM e C. **Nota:** passam sem o `@MainWrapper` (o JIT chama `eiwa_test_main` direto e o neco funciona para esses casos simples); o `@MainWrapper` (65.4-65.7) permanece pendente para paridade completa de entry point e remoção do macro C.
- [ ] **Verify 65:** `zig build test` + guardrail `passing_llvm` (LLVM e C); `eiwac build` de um programa com corrotinas linka neco; lib block custom do usuário com `@Source` + `@MainWrapper` funciona nos dois backends; `samples/main_wrapper_sample.ei` passa a imprimir `> before main` / `< after main` (wrapper Eiwa puro) nos dois backends. **Parcial:** `zig build test` e guardrail 53/53 ok; `@MainWrapper` pendente.
