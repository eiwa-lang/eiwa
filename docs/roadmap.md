...
>
> **Status medido (AGO 2026, binário `bin/eiwac` recompilado):** restam 3 arquivos em `failing_llvm` (estado real heterogêneo):
>
> - 💥 **Segfaults (3):** `http_test.ei` (0 pass — chamada HTTP GET), `serialization_test.ei` (1/5 — `serdeFields`/fat-pointer de contract), `task_test.ei` (0 pass — `Call parameter type does not match function signature` na corotina).
> - ✅ **Falhas de asserção zeradas.** Já corrigidos e promovidos: `id_test` (6/6), `money_test` (5/5), `log_test` (5/5), `std_json_parser_test` (5/5), `std_jsonrpc_test` (4/4 — resolvido de bônus pelo fix de `toDouble` da Task 64.10), `scope_functions_test` (13/13), `generic_methods_test` (6/6 — resolvido de bônus pelo fix de mangling da Task 64.13), `lambda_test` (9/9).
>
